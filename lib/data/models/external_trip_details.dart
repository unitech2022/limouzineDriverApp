import 'package:limousine_driver/data/models/user_response.dart';

import 'external_trip.dart';

class ExternalTripDetails {
  ExternalTrip? trip;
  List<BookingsResponse>? bookingsResponse;

  ExternalTripDetails({this.trip, this.bookingsResponse});

  ExternalTripDetails.fromJson(Map<String, dynamic> json) {
    trip = json['trip'] != null ?  ExternalTrip.fromJson(json['trip']) : null;
    if (json['bookingsResponse'] != null) {
      bookingsResponse = <BookingsResponse>[];
      json['bookingsResponse'].forEach((v) {
        bookingsResponse!.add(new BookingsResponse.fromJson(v));
      });
    }
  }

  
}


class BookingsResponse {
  Booking? booking;
  UserDetail? userDetail;

  BookingsResponse({this.booking, this.userDetail});

  BookingsResponse.fromJson(Map<String, dynamic> json) {
    booking =
        json['booking'] != null ? new Booking.fromJson(json['booking']) : null;
    userDetail = json['userDetail'] != null
        ? new UserDetail.fromJson(json['userDetail'])
        : null;
  }

}
class Booking {
  int? id;
  String? userId;
  int? driverId;
  int? externalTripId;
  int? status;
  String? createdAt;

  Booking(
      {this.id,
      this.userId,
      this.driverId,
      this.externalTripId,
      this.status,
      this.createdAt});

  Booking.fromJson(Map<String, dynamic> json) {
    id = json['id'];
    userId = json['userId'];
    driverId = json['driverId'];
    externalTripId = json['externalTripId'];
    status = json['status'];
    createdAt = json['createdAt'];
  }

  Map<String, dynamic> toJson() {
    final Map<String, dynamic> data = new Map<String, dynamic>();
    data['id'] = this.id;
    data['userId'] = this.userId;
    data['driverId'] = this.driverId;
    data['externalTripId'] = this.externalTripId;
    data['status'] = this.status;
    data['createdAt'] = this.createdAt;
    return data;
  }
}

